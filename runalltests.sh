...
$CC $OPTS --std=c++${YR} test_hash_speed.cpp crc32.cpp fnv_hash.cpp -lm \
  -lstdc++ >> $L 2>&1
./a.out 0 10000 >> $L 2>&1

rm -f a.out *.o

$CC $OPTS -std=c++17 test_ru_shared_mutex_speed.cpp ru_shared_mutex.cpp \
  -lstdc++ >> $L 2>&1
./a.out >> $L 2>&1

rm -f a.out *.o

$CC $OPTS -std=c++17 test_ru_shared_mutex_speed2.cpp ru_shared_mutex.cpp \
  -lstdc++ >> $L 2>&1
./a.out >> $L 2>&1
